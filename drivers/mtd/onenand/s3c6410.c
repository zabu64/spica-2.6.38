...
struct s3c6410_onenand {
	...
	struct s3c6410_onenand_transfer		pending_transfer;
};
				...
				enum s3c2410_dma_buffresult result)
{
	...
	if (dev_id) {
		struct s3c6410_onenand_transfer *oob = dev_id;
...
{
	struct device *dev = &onenand->pdev->dev;
	unsigned int flags = INT_ACT, dma = 0;
	...
	if (dma && (!wait_for_completion_timeout(&onenand->complete,
...
#else /* CONFIG_MTD_ONENAND_S3C6410_DMA */
static int s3c6410_onenand_command(struct mtd_info *mtd, int cmd, loff_t addr,
			       size_t len)
{
	struct onenand_chip *this = mtd->priv;
	...
	case ONENAND_CMD_READ:
		...
		__asm__ (
			...
			:
			: "r"(mcount / 8),
				"r"(onenand->ahb_addr + cmd_map_01), "r"(m)
	...
	case ONENAND_CMD_READOOB:
		...
		__asm__ (
			...
			:
			: "r"(scount / 8),
				"r"(onenand->ahb_addr + cmd_map_01), "r"(s)
			: "r0", "r1", "r2", "r3", "r4", "r5", "r6", "r7"
		);
#else
		{
	...
	case ONENAND_CMD_PROG:
		...
		__asm__ (
			...
			:
			: "r"(mcount / 8),
				"r"(m), "r"(onenand->ahb_addr + cmd_map_01)
			: "r0", "r1", "r2", "r3", "r4", "r5", "r6", "r7"
		);
#else
		{
	...
	case ONENAND_CMD_PROGOOB:
		...
		__asm__ (
			...
			"\tbne 1b\n"
			:
			: "r"(mcount / 8), "r"(onenand->ahb_addr + cmd_map_01)
			: "r0", "r1", "r2", "r3", "r4", "r5", "r6", "r7"
		);
#else
		{
		...
		__asm__ (
			...
			:
			: "r"(scount / 8),
				"r"(s), "r"(onenand->ahb_addr + cmd_map_01)
			: "r0", "r1", "r2", "r3", "r4", "r5", "r6", "r7"
		);
#else
		{
...
#endif /* CONFIG_MTD_ONENAND_S3C6410_DMA */
...
#endif /* CONFIG_MTD_ONENAND_S3C6410_PREFETCH */
...
static int s3c6410_onenand_probe(struct platform_device *pdev)
{
	...
	onenand->dummy_buf = kmalloc(SZ_4K, GFP_KERNEL);
