...
static struct clk *tin3;
static struct clk *tin4;
static struct clk *tdiv3;
...
static int s3c64xx_pwm_set_next_event(unsigned long cycles,
					struct clock_event_device *evt)
{
	s3c64xx_pwm_init(3, cycles);
	s3c64xx_pwm_start(3, 0);
	return 0;
}
				...
				struct clock_event_device *evt)
{
	...
	case CLOCK_EVT_MODE_PERIODIC:
		s3c64xx_pwm_init(3, clock_count_per_tick);
		s3c64xx_pwm_start(3, 1);
		break;
...
static struct clock_event_device pwm_event_device = {
	...
	.rating		= 200,
	.shift		= 32,
	.set_next_event	= s3c64xx_pwm_set_next_event,
...
static cycle_t s3c64xx_pwm4_read(struct clocksource *cs)
{
...
static void __init s3c64xx_timer_init_common(void)
{
	...
	if (IS_ERR(tin3))
		panic("failed to get pwm-tin3 clock for system timer");
